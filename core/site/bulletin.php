...
function createBulletin($authorId, $postContent) 
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($postContent['submit'])) {
        ...
        $allowedDurations = array(1, 3, 5, 7, 10);
        $duration = isset($postContent['duration']) ? (int) $postContent['duration'] : 10;
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO bulletins (title, text, author, date, expires_at) VALUES (?, ?, ?, ?, ?)");

            $stmt->bindParam(1, $title, PDO::PARAM_STR);
            $stmt->bindParam(2, $text, PDO::PARAM_STR);
            $stmt->bindParam(3, $author, PDO::PARAM_INT);
            $stmt->bindParam(4, $date, PDO::PARAM_STR);
            $stmt->bindParam(5, $expiresAt, PDO::PARAM_STR);
...
function fetchAllBulletins($limit=null) {
    global $conn;
    $query = "SELECT * FROM `bulletins` WHERE expires_at IS NULL OR expires_at > NOW() ORDER BY date DESC";
    if ($limit !== null) {
        $query .= " LIMIT :limit";
    } 
